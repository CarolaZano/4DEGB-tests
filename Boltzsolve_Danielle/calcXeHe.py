...
xstep,xinterp,etaans,etainterp=np.loadtxt('./eta.txt',unpack=True)
numx=len(xstep)
...
for i in range(0,10):
    ...
    np.savetxt('./fenew.txt',fenewsave)
    fe=fenew
...
np.savetxt('./Xevars.txt',savethese)

savethese2=np.column_stack((xplot,taudotplot))
np.savetxt('./XevarsHeplot.txt',savethese2)
