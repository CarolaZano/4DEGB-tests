...
###################################################### Initial conditions, tight coupling #############################################
    ...
    ddphi_detai = 0.    ### NOTE: FOR NOW ASSUME D(DPHI)/DETA = CONST = 0
    dphidoti = ddphi_detai/cf.H1(xinit,params)  ### Assume const ddphi_detai
    dphii = dphidoti + 0.     ### Assume const ddphi_detai, and dphii_0 = 0
...
################################################## Save Power Spectrum Values #######################################################
    ...
    matterpowertestnew=(((delmtot-3*Hconf*vmtot/k)*OmM+(delbtot-3*Hconf*vbtot/k)*OmB)/(OmM+OmB))**2*2.*np.pi**2*h**3/k**3*(A_s109*1.e-9*np.exp((n_s-1.)*np.log(k/0.05)))
    ...
    return np.array([k,matterpowertestnewtoday,transfermattertoday,transfercdm0,transferbar0,dphiwithk])
	
results = Parallel(n_jobs=4)(delayed(process)(k) for k in tqdm(kload2))

################################################## Output to file ###################################################################
results = np.array(results)
kvec = results.T[0]
matterpowertestnewtoday = results.T[1] 
transfermattertoday= results.T[2]
transfercdm0= results.T[3]
transferbar0= results.T[4]
dphiwithk = results.T[5]

### FOR TESTING ###
dphidotwithksave =np.column_stack((kvec,dphiwithk))
np.savetxt('./4DEGB_version/4DEGBTESTS.txt',dphidotwithksave)
###################
