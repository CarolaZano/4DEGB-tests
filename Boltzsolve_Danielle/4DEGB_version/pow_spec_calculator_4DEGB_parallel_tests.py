...
alpha_C_tilde,HH0,Nnu,wM0,wB0=np.loadtxt('./4DEGB_version/UniverseVars.txt',unpack=True)
...
OmN=params[15]/((params[10]+params[12])*aval+params[11]+params[15]+params[13]*aval**4) # dimensionless energy density for neutrinos + effect due to alpha_C
#CHANGED added GR version without alpha_C
OmNGR=params[16]/((params[10]+params[12])*aval+params[11]+params[15]+params[13]*aval**4) # dimensionless energy density for neutrinos only
OmL=1.-OmB-OmM-OmR-OmN # correct in 4DEGB as well
Omtot=OmB+OmM+OmR+OmN # might be incorrect in 4DEGB, but we don't use it anywhere
Omrad=OmR+OmN # includes alpha_C
Ommat=OmM+OmB
redshift=1./aval-1.
...
Hder=H0*(-(OmM0+OmB0)*np.exp(-xstep)-2.*(OmR0+OmN0)*np.exp(-2.*xstep)+2.*OmL0*np.exp(2.*xstep))/((OmM0+OmB0)*np.exp(-xstep)+(OmR0+OmN0)*np.exp(-2.*xstep)+OmL0*np.exp(2.*xstep))**0.5/2.

#Non interp
Hder2=H0*(-(OmM0+OmB0)*np.exp(-xinterp)-2.*(OmR0+OmN0)*np.exp(-2.*xinterp)+2.*OmL0*np.exp(2.*xinterp))/((OmM0+OmB0)*np.exp(-xinterp)+(OmR0+OmN0)*np.exp(-2.*xinterp)+OmL0*np.exp(2.*xinterp))**0.5/2.
...
def derivtightlycoupled(x,z,params,k):
	...
	#phiv= (-72.*(H0**2)*alpha_C*(OmR0*theta2 + OmN0GR*n2)/((k**2)*aval**4) -6.*alpha_C*Hconf*dphiprime/aval**2 -1.5*(Hconf**2 - alpha_C/aval**2)*(OmM*(Delm-3.*Hconf*vm/k)+4.*OmR*(T0+3.*Hconf*theta1/k)+OmB*(Delb-3.*Hconf*vb/k)+4*OmNGR*(N0+3*Hconf*n1/k))) / (k**2 + (Hconf**2 - alpha_C/aval**2)*(4.5*OmM+6.*OmR+4.5*OmB+6*OmNGR) - 6.*alpha_C/aval**2)
	...
	#Use an analytically reduced version of equation 33 which we have manipulated to reduce errors
	...
	derivatives=[Delmprime,vmprime,Delbprime,vbprime,T0prime,theta1prime,N0prime,n1prime,n2prime,n3prime,n4prime,n5prime,n6prime,n7prime,n8prime,n9prime,n10prime,dphiprimeprime,dphiprime,phiprimev]

	return derivatives

######################################### Regular DE solver ##################################################################

def deriv(x,z,params,k):
...
indexk=0
for k in tqdm(np.array([kload2[0],kload2[0]])):
    print( "k=", k)
...
###################################################### Initial conditions, tight coupling #############################################
    ...
    Cinit=-0.5  
    psii=20.*Cinit/(15.+4.*fv) # Does it matter what this is? Just normalization constant
    phii=(1.+0.4*fv)*psii # Checked - same in 4DEGB
    # CHANGED added this for the additional scalar field
    Hconfxdphidoti = 0. # O(eta^2)

    # CHANGED theta0i=-0.5*psii
    theta0i = -0.5*((OmR0+OmN0GR-alpha_C_tilde)*psii/(OmR0+OmN0GR) + 2.*alpha_C_tilde*Hconfxdphidoti/(OmR0+OmN0GR))
    ...
    theta1i=k/3./cf.H1(xinit,params)*psii * ((OmR0+OmN0)/(2*(OmR0+OmN0GR)) - alpha_C_tilde*Hconfxdphidoti/(OmR0+OmN0GR)/psii)
    
    ### FOR TESTING BCs VALIDITY (technically * eta for O(eta^2), but works like this too ####
    #Hconfxdphidoti = -100*theta1i
    #theta0i = -0.5*((OmR0+OmN0GR-alpha_C_tilde)*psii/(OmR0+OmN0GR) + 2*alpha_C_tilde*Hconfxdphidoti/(OmR0+OmN0GR))
    #T0i=theta0i-psii
    #delmi = 3*theta0i
    #Delmi=delmi-3*psii
    #delbi=delmi
    #Delbi=Delmi
    #theta1i=k/6./cf.H1(xinit,params)*psii * ((OmR0+OmN0)/(2*(OmR0+OmN0GR)) - alpha_C_tilde*Hconfxdphidoti/(OmR0+OmN0GR)/psii)
    ##########################################################################################
    ...
    N0i= T0i
    n1i=theta1i
    n2i=k**2*np.exp(2.*xinit)*phii/((12.*H0**2*OmN0GR)*((5./(2.*fv))+1.)) # Checked - same in 4DEGB
    n3i=k/(7.*cf.H1(xinit,params))*n2i # Checked - same in 4DEGB
    n4i=k/(9.*cf.H1(xinit,params))*n3i # Checked - same in 4DEGB
    n5i=k/(11.*cf.H1(xinit,params))*n4i # Checked - same in 4DEGB
    n6i=k/(13.*cf.H1(xinit,params))*n5i # Checked - same in 4DEGB
    n7i=k/(15.*cf.H1(xinit,params))*n6i # Checked - same in 4DEGB
    n8i=k/(17.*cf.H1(xinit,params))*n7i # Checked - same in 4DEGB
    n9i=k/(19.*cf.H1(xinit,params))*n8i # Checked - same in 4DEGB
    n10i=k/(21.*cf.H1(xinit,params))*n9i # Checked - same in 4DEGB
    
    # CHANGED added this for the additional scalar field
    dphii = Hconfxdphidoti/(3.*cf.H1(xinit,params))
    dphidoti = Hconfxdphidoti/cf.H1(xinit,params)

############################################ Solve the ODE, tight coupling #############################################################
    #print 'begin tight couple'
    # CHANGED to include additional 4DEGB scalar field degrees of freedom
    inittight=[Delmi,vmi,Delbi,vbi,T0i,theta1i,N0i,n1i,n2i,n3i,n4i,n5i,n6i,n7i,n8i,n9i,n10i,dphidoti,dphii,phii]
    # CHANGED not to get divergences for high k
    #soltight=scipy.integrate.odeint(derivtightlycoupled,inittight,xtight,args=(params,k))
    soltight=scipy.integrate.solve_ivp(derivtightlycoupled,(xtight[0],xtight[-1]),np.array(inittight),args=(params,k),t_eval=xtight, method ='BDF').y.T
...
############################################ Solve the ODE, regular #############################################################
    ...
    initreg=[Delmi2,vmi2,Delbi2,vbi2,T0i2,theta1i2,N0i2,n1i2,theta2i2,theta3i2,theta4i2,theta5i2,theta6i2,theta0pi2,theta1pi2,theta2pi2,theta3pi2,theta4pi2,theta5pi2,theta6pi2,n2i2,n3i2,n4i2,n5i2,n6i2,n7i2,n8i2,n9i2,n10i2,dphidoti2,dphii2,phii2]
    #print 'begin solve reg ode'
    # CHANGED not to get divergences for high k
    #solreg=scipy.integrate.odeint(deriv,initreg,xreg,args=(params,k))
    solreg=scipy.integrate.solve_ivp(deriv,(xreg[0],xreg[-1]),np.array(initreg),args=(params,k), t_eval= xreg, method ='BDF').y.T
...
############################################ Patch regimes together ##############################################################
    ...
    phicon = np.append(soltight[:,19],solreg[:,31])
    #Reconstruct phias
    ...
    #phicon=(-72.*(H0**2)*alpha_C*(OmR0*theta2tot + OmN0GR*n2tot)/((k**2)*np.exp(4*xstep)) -6.*alpha_C*Hconf*dphidottot/np.exp(2*xstep) -1.5*(Hconf**2 - alpha_C/np.exp(2*xstep))*(OmM*(Delmtot-3.*Hconf*vmtot/k)+4.*OmR*(T0tot+3.*Hconf*theta1tot/k)+OmB*(Delbtot-3.*Hconf*vbtot/k)+4*OmNGR*(N0tot+3*Hconf*n1tot/k))) / (k**2 + (Hconf**2 - alpha_C/np.exp(2*xstep))*(4.5*OmM+6.*OmR+4.5*OmB+6*OmNGR) - 6.*alpha_C/np.exp(2*xstep))
...
################################################## Save Power Spectrum Values #######################################################
    ...
    transfermattertoday=transfermatter[-1]
    transfercdm0=transfercdm[-1]
    transferbar0=transferbar[-1]
    matterpowertestnewtoday=matterpowertestnew[-1]
    kvec=k
    phiofk=phicon[-1]
...
### FOR TESTING ###
paramswith_a_save =np.column_stack((xstep,phicon,dphitot))
np.savetxt('./4DEGB_version/4DEGBTESTS.txt',paramswith_a_save)
###################
