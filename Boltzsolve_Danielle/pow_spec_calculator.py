...
xstep,xinterp,Xetot,ne,taudot,etaans,etainterp=np.loadtxt('./Xevars.txt',unpack=True)
numx=len(xstep)
...
def derivtightlycoupled(z,x,params):
	...
	phiv=-1.5*Hconf**2*(OmM*(Delm-3.*Hconf*vm/k)+4.*OmR*(T0+3.*Hconf*theta1/k)+OmB*(Delb-3.*Hconf*vb/k)+4*OmN*(N0+3*Hconf*n1/k))/(k**2+4.5*Hconf**2*OmM+6.*Hconf**2*OmR+4.5*Hconf**2*OmB+6*Hconf**2*OmN)
	psiv=phiv-12.*H0**2/k**2/aval**2*(OmR0*theta2+OmN0*n2)
	phiprimev=-psiv-kovH**2/(3.)*phiv-H0**2/(2.*Hconf**2)*(OmM0/aval*(Delm+3.*phiv)+OmB0/aval*(Delb+3.*phiv)+4.*OmR0/aval**2*(T0+phiv)+4.*OmN0/aval**2*(N0+phiv))
	...
	#Return
	derivatives=[Delmprime,vmprime,Delbprime,vbprime,T0prime,theta1prime,N0prime,n1prime,n2prime,n3prime,n4prime,n5prime,n6prime,n7prime,n8prime,n9prime,n10prime]
...
def deriv(z,x,params):
	...
	phiv=-1.5*Hconf**2*(OmM*(Delm-3.*Hconf*vm/k)+4.*OmR*(T0+3.*Hconf*theta1/k)+OmB*(Delb-3.*Hconf*vb/k)+4*OmN*(N0+3*Hconf*n1/k))/(k**2+4.5*Hconf**2*OmM+6.*Hconf**2*OmR+4.5*Hconf**2*OmB+6.*Hconf**2*OmN)
	psiv=phiv-12*H0**2/k**2/aval**2*(OmR0*theta2+OmN0*n2)
	phiprimev=-psiv-k**2/(3.*Hconf**2)*phiv-H0**2/(2.*Hconf**2)*(OmM0/aval*(Delm+3.*phiv)+OmB0/aval*(Delb+3.*phiv)+4.*OmR0/aval**2*(T0+phiv)+4*OmN0/aval**2*(N0+phiv))
...
###################################################### Initial conditions, tight coupling #############################################
    ...
    delbi=delmi
    Delbi=Delmi
    theta1i=k/6./cf.H1(xinit,params)*psii
    vmi=-k/2./cf.H1(xinit,params)*psii
    vbi=vmi
    N0i=-1.5*psii
    n1i=theta1i
    n2i=k**2*np.exp(2.*xinit)*phii/((12.*H0**2*OmN0)*((5./(2.*fv))+1.))
...
############################################ Solve the ODE, tight coupling #############################################################
    ...
    inittight=[Delmi,vmi,Delbi,vbi,T0i,theta1i,N0i,n1i,n2i,n3i,n4i,n5i,n6i,n7i,n8i,n9i,n10i]
...
############################################ Solve the ODE, regular #############################################################
    ...
    initreg=[Delmi2,vmi2,Delbi2,vbi2,T0i2,theta1i2,N0i2,n1i2,theta2i2,theta3i2,theta4i2,theta5i2,theta6i2,theta0pi2,theta1pi2,theta2pi2,theta3pi2,theta4pi2,theta5pi2,theta6pi2,n2i2,n3i2,n4i2,n5i2,n6i2,n7i2,n8i2,n9i2,n10i2]
...
############################################ Patch regimes together ##############################################################
    ...
    phicon=-1.5*Hconf**2*(OmM*(Delmtot-3.*Hconf*vmtot/k)+4.*OmR*(T0tot+3.*Hconf*theta1tot/k)+OmB*(Delbtot-3.*Hconf*vbtot/k)+4.*OmN*(N0tot+3.*Hconf*n1tot/k))/(k**2+4.5*Hconf**2*OmM+6.*Hconf**2*OmR+4.5*Hconf**2*OmB+6.*Hconf**2*OmN)

    psicon=phicon-12*H0**2/k**2/np.exp(2*xstep)*(OmR0*theta2tot+OmN0*n2tot)
...
################################################## Save Power Spectrum Values #######################################################
    ...
    matterpowertestnew=(((delmtot-3*Hconf*vmtot/k)*OmM+(delbtot-3*Hconf*vbtot/k)*OmB)/(OmM+OmB))**2*2.*np.pi**2*h**3/k**3*(A_s109*10.**(-9)*np.exp((n_s-1.)*np.log(k/0.05)))
...
np.savetxt('./matterpower.txt',matterpowertestnewsave)

transfersave=np.column_stack((kvec,transfermattertoday))
np.savetxt('./transfermatter.txt',transfersave)

transfercdmsave=np.column_stack((kvec,transfercdm0))
np.savetxt('./transfercdm.txt',transfercdmsave)

transferbarsave=np.column_stack((kvec,transferbar0))
np.savetxt('./transferbar.txt',transferbarsave)
